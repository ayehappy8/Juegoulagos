...
def subir_nivel():
	...
	texto_intro = pygame.font.SysFont('comicsans',100)
...
while repetir:
	...
	texto_intro = pygame.font.SysFont('Footlight MT Light', 40, True)
	texto_resultado = pygame.font.SysFont('console', 80, True)
	esta_en_intro = True
	gana = False
	personaje_intro = personaje(50,150,"heroe",700)
	#Creación Personaje Héroe
	heroe=personaje(int(ventana_x/2), int(ventana_y/2),"heroe",10000)#Agregar límite

	villanos = [personaje(130, int(ventana_y - 300),"villano",800),personaje(275, int(ventana_y - 150),"villano2",400), personaje(30, int(ventana_y - 350),"villano3",600),  personaje(80, int(ventana_y - 400),"villano4",600)]
	villano= villanos[nivel]
	...
	while esta_en_intro:
		...
		ventana.blit(pygame.image.load('img/Menu/main_menu.jpg'),[-3,-48]) # pinta el fondo de negro
...
# Seccion de pantalla final
	...
	while final:
		...
		if gana:
			ventana.blit(pygame.image.load('img/Menu/ganado.jpg'),[0,0])
			pts = texto_intro.render('Puntaje Total: '+str(puntaje), 1, (117, 174, 5))
			ventana.blit(pts,(300,250))
			instrucciones = texto_intro.render('Presione ENTER para cerrar...', 1, (84, 255, 218))
			reintentar = texto_intro.render('Presione R para volver al juego...', 1, (84, 255, 218))
			...
			ventana.blit(pygame.image.load('img/Menu/perdiste.jpg'),[0,20])
			pts = texto_intro.render('Puntaje Total: '+str(puntaje), 1, (226, 9, 137))
			ventana.blit(pts,(540,350))
		pygame.display.update()
